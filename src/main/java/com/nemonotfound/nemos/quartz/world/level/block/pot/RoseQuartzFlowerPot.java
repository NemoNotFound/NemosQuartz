...
public class RoseQuartzFlowerPot extends FlowerPotBase {

    public static final MapCodec<RoseQuartzFlowerPot> CODEC = RecordCodecBuilder.mapCodec(
            (instance) -> instance.group(BuiltInRegistries.BLOCK.byNameCodec()
                            .fieldOf("potted")
                            .forGetter(FlowerPotBase::getPotted), propertiesCodec()
                    )
                    .apply(instance, RoseQuartzFlowerPot::new)
    );
    private static final Map<Block, Block> POTTED_BY_CONTENT = Maps.newHashMap();

    public RoseQuartzFlowerPot(Block potted, Properties properties) {
    ...
    public Block getFlowerPot() {
        return QuartzBlocks.ROSE_QUARTZ_FLOWER_POT;
    }
}
