...
public abstract class FlowerPotBase extends Block {

    private static final VoxelShape SHAPE = Block.column(6.0F, 0.0F, 6.0F);
    private final Block potted;

    public FlowerPotBase(Block potted, Properties properties) {
        super(properties);
        this.potted = potted;
    }
    ...
    protected @NotNull InteractionResult useItemOn(ItemStack stack, @NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, @NotNull Player player, @NotNull InteractionHand hand, @NotNull BlockHitResult hitResult) {
        ...
        if (stack.getItem() instanceof BlockItem blockItem) {
            pottedBlock = getPottedByContent().getOrDefault(blockItem.getBlock(), Blocks.AIR);
        } else {
    ...
    protected abstract Map<Block, Block> getPottedByContent();
    ...
    public abstract Block getFlowerPot();
