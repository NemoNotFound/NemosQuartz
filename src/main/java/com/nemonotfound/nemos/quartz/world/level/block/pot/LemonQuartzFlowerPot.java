...
public class LemonQuartzFlowerPot extends FlowerPotBase {

    public static final MapCodec<LemonQuartzFlowerPot> CODEC = RecordCodecBuilder.mapCodec(
            (instance) -> instance.group(BuiltInRegistries.BLOCK.byNameCodec()
                            .fieldOf("potted")
                            .forGetter(FlowerPotBase::getPotted), propertiesCodec()
                    )
                    .apply(instance, LemonQuartzFlowerPot::new)
    );
    private static final Map<Block, Block> POTTED_BY_CONTENT = Maps.newHashMap();

    public LemonQuartzFlowerPot(Block potted, Properties properties) {
    ...
    public Block getFlowerPot() {
        return QuartzBlocks.LEMON_QUARTZ_FLOWER_POT;
    }
}
